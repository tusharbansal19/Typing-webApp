...
const Hostpage = ({ darkMode }) => {
  ...
  const RoomCreationModal = () => {
    ...
    const OptionButton = ({ active, onClick, children, label }) => (
      <button
        onClick={onClick}
        className={`flex flex-col items-center justify-center p-3 rounded-lg border transition-all duration-200 w-full ${active
            ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300'
            : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'
          }`}
      >
        <span className={`font-medium ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>{children}</span>
        {label && <span className={`text-xs mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{label}</span>}
      </button>
    );
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className={`relative w-full max-w-xl max-h-[90vh] overflow-y-auto rounded-xl shadow-xl ${darkMode
          ...
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
            ...
            <button
              onClick={() => setShowCreateModal(false)}
              className={`p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors ${darkMode ? 'text-gray-400' : 'text-gray-500'
                }`}
            >
          ...
          <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-6 gap-2">
                {[2, 4, 6, 8, 10, 12].map((limit) => (
                  <button
                    ...
                    className={`p-2 rounded-md text-sm font-medium transition-all ${roomConfig.playerLimit === limit
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {['easy', 'normal', 'hard'].map((level) => (
                  <OptionButton
                    key={level}
                    active={roomConfig.difficulty === level}
            ...
            {/* Toggles */}
            <div className="flex flex-col gap-4 p-4 rounded-lg bg-gray-50 dark:bg-gray-800/50">
              <div className="flex items-center justify-between">
                <div>
                  <div className={`text-sm font-medium ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>Private Room</div>
                  <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Requires invite code to join</div>
                </div>
                <button
                  onClick={() => setRoomConfig(prev => ({ ...prev, isPrivate: !prev.isPrivate }))}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${roomConfig.isPrivate ? 'bg-indigo-600' : 'bg-gray-300 dark:bg-gray-600'
                    }`}
                >
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className={`text-sm font-medium ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>Spectators</div>
                  <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Allow others to watch</div>
                </div>
                <button
                  onClick={() => setRoomConfig(prev => ({ ...prev, allowSpectators: !prev.allowSpectators }))}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${roomConfig.allowSpectators ? 'bg-indigo-600' : 'bg-gray-300 dark:bg-gray-600'
                    }`}
                >
          ...
          <div className="p-6 border-t border-gray-200 dark:border-gray-800 flex gap-3">
            ...
            <button
              ...
              disabled={loading}
              className="flex-1 py-2.5 px-4 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
  ...
  return (
    <main className={`min-h-screen py-12 transition-colors duration-500 ${
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8">
            ...
            {createdRoom && (
              <div className={`mt-6 p-5 rounded-xl border ${
                ...
                <div className="flex items-center gap-2 mb-4">
                  <code className={`flex-1 p-3 rounded-lg font-mono text-center font-bold tracking-wider ${
                    darkMode ? 'bg-black/30 text-indigo-400' : 'bg-white text-indigo-600 border border-gray-200'
                  }`}>
                    {createdRoom}
                  </code>
                  <button
          ...
          <section className={`p-8 rounded-2xl border transition-all duration-300 ${
            ...
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 transition-colors ${
              darkMode ? 'bg-pink-500/10 text-pink-400' : 'bg-pink-50 text-pink-600'
            }`}>
              <FaSignInAlt size={20} />
            </div>
            ...
            <div className="space-y-4">
              <div className="relative">
                <input
                  ...
                  className={`w-full p-3 pl-4 rounded-lg outline-none border transition-all ${
                    darkMode 
